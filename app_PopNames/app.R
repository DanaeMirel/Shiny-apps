...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
            tabsetPanel(
                tabPanel('Plot', 
            plotlyOutput('plot_trendy_names')), 
                tabPanel('Table', 
            tableOutput('table_trendy_names')))
            ) 
        )
    )
...
server <- function(input, output, session) {
    ...
    output$table_trendy_names <- function()({
        ...
        get_top_names() %>% 
            select(name, n, prop) %>%  
            #mutate(name = cell_spec(name, color = "white", bold = T
            #       ,background = spec_color(1:10, begin = 0.25, end = 0.5, option = "A", direction = -1))) %>% 
            ...
            kable_styling(full_width = F
                          ...
                          ,position = "left") %>% column_spec(2:3, bold = T)
